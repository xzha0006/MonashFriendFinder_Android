...
/**
 * This is the search map fragment
...
public class SearchMapFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        String strangers = sp.getString("strangers", null);
        String strangerIds = sp.getString("strangerIds", "");
        final JsonArray strangersJson = new JsonParser().parse(strangers).getAsJsonArray();
                ...
                protected void onPostExecute(final JsonArray locations) {
                    ...
                    mMapView.getMapAsync(new OnMapReadyCallback() {
                        ...
                        public void onMapReady(MapboxMap mapboxMap) {
                            ...
                            //loop to add marker
                            if (strangersJson.size() > 0 && locations.size() > 0) {
                                //use two loop to match the locations
                                for (int i = 0; i < strangersJson.size(); i++) {
                                    JsonObject friend = strangersJson.get(i).getAsJsonObject();
